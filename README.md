...
### Описание
Веб-приложение для отслеживания привычек. Перенос клиентской части в NestJS с шаблонизацией EJS и деплоем на Render.

### Функционал
- 9 страниц (главная, привычки, прогресс, галерея и др.)
- Адаптивная верстка
- Два состояния сессии (`?auth=true` / `?auth=false`)
- Выделенные partials (header, nav, footer, session)
...
views/ # EJS шаблоны (включая partials)
src/ # Контроллеры и настройки
